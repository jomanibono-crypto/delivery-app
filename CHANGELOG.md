...
## v1.9.5+47 — CI pipeline fix

- إصلاح سلسلة النشر الآلي على GitHub Actions: الـ release APK كان يفشل بسبب عدم توقيع الحزمة (missing `storeFile`) لأن الـ keystore لم يكن متاحاً في الـ CI. الآن يتم استرجاع الـ keystore من GitHub secrets أثناء البناء.
- نفس الميزات الموجودة في v1.9.4+46 (علامات "زبون سيئ" الدائمة على الخريطة).

## v1.9.4+46 — Persistent "bad customer" map markers

- **علامات "زبون سيئ" الدائمة على الخريطة**: اضغط مطولاً على أي نقطة في الخريطة، اختر "زبون سيئ"، وأضف علامة حمراء صغيرة تبقى في مكانها بشكل دائم حتى يتم حذفها يدوياً. يمكن إضافة اسم الزبون، رقم الهاتف، والسبب (كلها اختيارية).
- اضغط على العلامة لعرض التفاصيل (الاسم، الهاتف، السبب، من أضافها، التاريخ) أو لحذفها.
- العلامات تظهر لجميع المجموعات (عامّة)، تماماً مثل قائمة الحظر الحالية للهواتف.
- النموذج `BlacklistEntry` تم توسيعه بحقول `lat`/`lng`/`name` (اختيارية)؛ الإدخالات القديمة (الهواتف فقط) تظل تعمل دون تغيير.
